...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $selectedDepartmentId = isset($_POST['departmentId'])?$_POST['departmentId']:'';
    ...
    $query = "SELECT p.id, p.firstName, p.lastName, p.jobTitle, p.email, p.departmentID, d.name as department, l.name as location FROM personnel p INNER JOIN department d ON p.departmentID = d.id INNER JOIN location l ON d.locationID = l.id WHERE 1 = 1 "; // Starting with a basic query
