$(document).ready(function (e) {
  var markerLayer; // Declare the markerLayer variable
  ...
  var Basemaps = {
    "Streets": streets,
    "Satellite": satellite,
    "OpenStreetMap": tiles, // Add the OpenStreetMap layer
  };

  var airportMarkers = L.markerClusterGroup();
  var cityMarkers = L.markerClusterGroup();

  var layerControl = L.control.layers(Basemaps, { 'Airports': airportMarkers, 'Cities': cityMarkers}).addTo(map);
  ...
  L.easyButton("fa-solid fa-tv", function (btn, map) {
    if (selectedCountryData) {
  ...
  function customTip() {
    this.unbindTooltip();
    if (!this.isPopupOpen()) this.bindTooltip("Short Description Data").openTooltip();
  }
  ...
  function fetchCountryDetails(iso_a2) {
    $.ajax({
      type: "POST",
      ...
      success: function (data) {
        selectedCountryData = data[0]; // Save the data globally
        selectedCountryData.iso_a2 = iso_a2;
// Fetch weather data here too
        $.ajax({
          url: "https://coding.itcareerswitch.co.uk/weatherapi/getWeather.php",
          type: 'POST',
          dataType: 'json',
          data: {
            location: data[0].capital+','+iso_a2
          },
          success: function (result) {
            ...
            // Save weather data in the global variable too
            // selectedCountryData.weather = response;
...
$('.language').html(languages);
            ...
            $.ajax({
              ...
              data: { country: iso_a2 },
              dataType: "json",
              beforeSend:function(){$('.news-body').html('fetching news ...')},
              success: function (response) {
                ...
                if(articles.length > 0) {
                  for (var i = 0; i < 5; i++) {
                    if (articles[i]) {
                     ...
                     <table class="table table-borderless">
                            <tr>
                              <td>
                                <a href="`+articles[i].url+`" class="fw-bold fs-6 text-black" target="_blank">`+articles[i].title+`</a>
                              </td>
                            </tr>                            
                          </table>
                  ...
                  $('.news-body').html(`<p>No news updates for this country ...</p>`);
                }
              },
            });
          },
  ...
  function getCountries(selectedCountry) {
    ...
    if (Array.isArray(data.features)) {
      data.features.sort(function(a, b) {
        var keyA = a.properties.name,
            keyB = b.properties.name;
        // Compare the 2 dates
      ...
      select.on("change", function () {
        ...
        // add 7 more markers for air port
        if (selectedFeature) {
          getAirports(selectedCountry);
        ...
        selectedLayer.addTo(map).bringToBack();

        var countryLatLng = selectedLayer.getBounds().getCenter();
        if (markerLayer) {
        ...
        markerLayer = L.marker(countryLatLng, {icon: greenIcon})
            .addTo(map)
            .on("click", function () {
              fetchCountryDetails(selectedFeature.properties.iso_a2);
              $("#detailsmodal").modal("show"); // Open the modal when marker is clicked
            });
  ...
  function onLocationFound(e) {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function (response) {
        var country = response.address.country_code.toUpperCase();
        getCountries(country);
      },
    });
  }
  function getAirports(country = false) {
    if (!country) {
      cntry = $("#country-select").val();
    } else {
      cntry = country;
    }
    ...
    $.ajax({
      ...
      headers: { 'X-Api-Key': 'JZL8dnai75NN+WeSveiwmQ==W3EY35hWlgUhrG0r' },
      contentType: 'application/json',
      beforeSend: function () {},
      success: function (data) {

        // Create a marker cluster group
        var markerCluster = airportMarkers;
        ...
        for (i = 0; i < data.length; i++) {
          if (i < 7) {
            ...
            marker.addTo(markerCluster); // Add marker to the cluster layer
          }
        }

        map.addLayer(markerCluster); // Add the marker cluster layer to the map
      },
    });
  }
  function getCities(country = false) {
    ...
    $.ajax({
      type: 'GET',
      url: 'https://airlabs.co/api/v9/cities?country_code='+country+'&api_key=ad28bdc7-1d8a-426f-9c63-a334704da199',
      contentType: 'application/json',
      beforeSend: function () {},
      success: function (data) {

        // Create a marker cluster group
        var markerCluster = cityMarkers;
        ...
        for (i = 0; i < data.response.length; i++) {
          if (i < 7) {
            var greenIcon = L.icon({
              ...
              shadowUrl: 'img/shadow.png',
              iconSize:     [57, 60], // size of the icon
              shadowSize:   [57, 47], // size of the shadow
              shadowAnchor: [17, 24],  // the same for the shadow
            });
            var markerOptions = {
              icon: greenIcon,
              title: data.response[i].name,
              clickable: true,
              draggable: true,
            };

            var marker = L.marker(
                [data.response[i].lat, data.response[i].lng],
                markerOptions
            );
            marker.addTo(markerCluster); // Add marker to the cluster layer
          }
  ...
  map.locate({ setView: true, watch: false, maxZoom: 2 });
